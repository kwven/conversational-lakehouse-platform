...
topic = Config.KAFKA_TOPIC + "departments"
...
def extract_departments(spark: SparkSession):
    df = IOUtils.read_stream(spark, topic)
    return df

def transform_departments(df: DataFrame):
    df_parse = df.select(
    ...
    df_transform= df_parse.select(
        ...
        F.coalesce(F.col("payload.after.department"),F.col("payload.before.department")).alias("department"),
...
def main():
    ...
    query = load_departments(transform_departments(extract_departments(spark)))
