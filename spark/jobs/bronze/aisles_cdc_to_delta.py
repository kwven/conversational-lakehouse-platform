...
topic = Config.KAFKA_TOPIC + "aisles"
...
def extract_aisles(spark: SparkSession):
    df = IOUtils.read_stream(spark, topic)
    return df

def transform_aisles(df: DataFrame):
    df_parse = df.select(
    ...
    df_transform= df_parse.select(
        ...
        F.coalesce(F.col("payload.after.aisle"),F.col("payload.before.aisle")).alias("aisle"),
...
def main():
    ...
    query = load_aisles(transform_aisles(extract_aisles(spark)))
