...
def transform(products,orders,departments,aisles,order_products_prior):
    ...
    df_clean = df_all.groupBy("aisle_id","aisle","department_id","department").agg(F.count("order_id").alias("total_items_sold"),
                                                               ...
                                                               F.countDistinct("product_id").alias("distinct_products"),                                                               
                                                                    ...
                                                                    F.dense_rank().over(
                                                                        Window.orderBy(
                                                                            F.col("total_items_sold").desc(),
                                                                            F.col("distinct_orders").desc(),
                                                                            F.col("unique_customers").desc()
                                                                        )
                                                                    )
                                                                ).withColumn("gold_created_at",F.current_timestamp())
    return df_clean
def load(table):
    return IOUtils.write_delta(table,"top_aisles_clean")
def main():
    spark = SparkSessionFactory.create("gold-top-aisles")
