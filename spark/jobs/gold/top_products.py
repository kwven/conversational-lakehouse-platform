...
def transform(products,orders,departments,aisles,order_products_prior):
    ...
    df_clean = df_all.groupBy("product_id","product_name","aisle_id","aisle","department_id","department").agg(F.count("*").alias("total_purchases"),
                                                               F.countDistinct("order_id").alias("distinct_orders"),
                                                               F.countDistinct("user_id").alias("unique_users"),
                                                               F.sum(F.col("reordered")).alias("total_reorders"),
                                                               F.round(F.avg(F.col("reordered") * 100), 2).alias("reordered_rate_pct")
                                                               ).withColumn(
                                                                    "product_rank",
                                                                    F.dense_rank().over(
                                                                        Window.orderBy(
                                                                            F.col("total_purchases").desc(),
    ...
    return df_clean
def load(table):
    return IOUtils.write_delta(table,"top_products_clean")
def main():
    spark = SparkSessionFactory.create("gold-top-products")
