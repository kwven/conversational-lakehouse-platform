...
def extract(spark):
    read = IOUtils.read_delta_stream(spark,bronze_table="bronze/products")
    return read

def transform(df:DataFrame,batch_id: int):
    if df.isEmpty():
        return 
    window = Window.partitionBy(F.col("product_id")).orderBy(F.col("_connector_ts_ms").desc_nulls_last(),F.col("ingested_at").desc_nulls_last())
    ...
    if not DeltaTable.isDeltaTable(spark,Config.SILVER_PATH + "products_clean"):
        (df_clean.select(
                "product_id",
                "product_name",
                "aisle_id",
                ...
                "ingested_at",          
                "is_deleted"        
            )).write.format("delta").mode("overwrite").save(Config.SILVER_PATH + "products_clean")
        return
    
    target = DeltaTable.forPath(spark, Config.SILVER_PATH + "products_clean")
...
def load(df_clean:DataFrame,merge_to_silver:Callable):
    return IOUtils.write_delta_stream(df_clean,merge_to_silver, Config.SILVER_PATH, "products_clean")
    
def main():
    spark = SparkSessionFactory.create("silver-products")
    df = extract(spark)
    load(df,transform)
